/*
 * JWKS session cache utility
 * - caches JWKS JSON in sessionStorage for a configurable TTL (seconds)
 * - exposes getLocalJwkSet(domain) which returns a jose LocalJWKSet function
 */
...
export async function getLocalJwkSet(domain: string) {
  ...
  if (inFlightFetches.has(domain)) {
    return await inFlightFetches.get(domain)!;
  }
  ...
  const promise = (async () => {
    try {
      ...
      try {
        ...
        console.warn("JWKS: sessionStorage unavailable, falling back to network", e);
      }
